...
for($ixx=0; $ixx<$xxnum; $ixx++){
  ...
  $sql_inst = "select * from db_config where codigo = ".$iInstituicao;
  ...
  //dados do sindicato
  $sSqlSindicato = "select rhsindicato.* 
  		              from rhpessoal 
  		                   inner join rhsindicato on rh116_sequencial = rh01_rhsindicato 
  		             where rh01_regist = {$rh01_regist}";
  $rsSindicato = db_query($sSqlSindicato);
  $oDadosSindicato = db_utils::fieldsMemory($rsSindicato, 0);
  
  $pdf1->sindicato_codigo    = $oDadosSindicato->rh116_codigo;
  $pdf1->sindicato_cnpj      = $oDadosSindicato->rh116_cnpj;
  $pdf1->sindicato_descricao = $oDadosSindicato->rh116_descricao;
